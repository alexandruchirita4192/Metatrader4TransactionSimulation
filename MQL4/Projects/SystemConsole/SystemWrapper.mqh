...
static GlobalVariableCommunication comm(true, false);
...
public:
	...
	int OnInitWrapper(string configValue);
...
int SystemWrapper::OnInitWrapper(string configValue)
{
   GlobalContext.Config.SetValue("ConfigFile", configValue); // NewTradingSession requires it
	...
	comm.Initialize(true, true); // init timers too
   ...
   //comm.Connect();
	...
	if (useEA)
	{
		...
		else // Use Automatic Decision EA - needs Database simulation data + run GetResults, WS call, etc
		{
			...
			while (!isTransactionAllowedOnChartTransactionData)
			{
				...
				if (lots.IsMarginOk(symbol, SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN), 0.4f, true) && GlobalContext.Config.IsTradeAllowedOnEA(symbol))
				{
					system.CleanTranData();
		...
		BaseLotManagement lots;
		if (lots.IsMarginOk(_Symbol, SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN), 0.4f, true))
		{
	...
	{
		GlobalContext.Config.Initialize(true, true, false, true, configValue);
		...
		{
			if (useDiscoverySystem || useLightSystem || useFullSystem) // before: useFullSystem only
			{
	...
	LogInfoMessageIfTrue(useLightSystem || useDiscoverySystem || useFullSystem,
	            "System run for " +
...
{
	// Bulk debug anyway
	GlobalContext.DatabaseLog.ParametersSet(
		GlobalContext.Config.GetSessionName(),
		__FUNCSIG__ + IntegerToString(reason),
		TimeAsParameter());
	GlobalContext.DatabaseLog.CallBulkWebServiceProcedure("BulkDebugLog", true, false);
...
{
   Print(__FUNCSIG__ + " beginning; CurrentSymbol=" + CurrentSymbol + "; _Symbol=" + _Symbol + "; AllowChanging=" + BoolToString(GlobalContext.Config.GetBoolValue("AllowChangingSymbol")));
...
void SystemWrapper::OnTimerWrapper()
{
	...
	{
	   Print(__FUNCSIG__ + " calling ExecuteCommand(" + word + ")");
		ExecuteCommand(word);
