...
void OnTick()
{
	...
	if((d != IncertitudeDecision) && (nrDecisions == 0))
	{
		...
		if(d < 0) { // Sell - inverse decision
			...
			//GlobalContext.Money.CalculateTP_SL(TP, SL, 2.6*spreadPips, 1.6*spreadPips, OP_BUY, price, _Symbol, spread);
			...
			//GlobalContext.Money.CalculateTP_SL(TP, SL, 2.6*spreadPips, 1.6*spreadPips, OP_SELL, price, _Symbol, spread);
