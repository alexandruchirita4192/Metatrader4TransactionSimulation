...
int OnInit()
{
	...
	for(int i=0;i<hstTotal;i++)
	{
		...
		/*
		printf("[%d]: realProfit=%f calculatedProfit=%f calculatedProfitStopLoss=%f calculatedProfitTakeProfit=%f orderComission=%f orderSwap=%f", i, 
		OrderProfit(), orderProfitTest, orderProfitStopLossTest, orderProfitTakeProfitTest, orderComission, orderSwap);
		*/
		...
		*/
		...
		SafePrintString("Symbol: " + OrderSymbol() + "; orderType[" + orderType + "];" + (orderType == "sell" ? "(openPrice[" + DoubleToStr(openPrice,4) + "]-closePrice[" + DoubleToStr(closePrice,4) + "])" : "(closePrice[" + DoubleToStr(closePrice,4)
		+ "]-openPrice[" + DoubleToStr(openPrice,4) +"])") + "*orderLots[" + DoubleToStr(orderLots,4)
		+ "]*changeRate[" + DoubleToStr(changeRate,4) + "]+orderComission[" + DoubleToStr(orderComission,4) + "]+orderSwap[" + DoubleToStr(orderSwap,4)
		+ "]=orderProfitTest[" + DoubleToStr(orderProfitTest,4) + "];realProfit[" + DoubleToStr(orderProfitReal,4) + "];SimulatedOrderProfit(close)[" + DoubleToStr(order.SimulatedOrderProfit(OrderCloseTime(), OrderClosePrice(), true),4) + "]");
