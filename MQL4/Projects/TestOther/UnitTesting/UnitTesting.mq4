...
bool TestOrderLimits(string &errors, int orderType = OP_BUY)
{
   ...
   errors = "";
	...
	GlobalContext.Limit.CalculateTP_SL(TP, SL, TpLimitPips, SlLimitPips, orderType, price, _Symbol, spread);
   if((TP == 0.0) || (SL == 0.0))
   {
      isOk = false;
	...
	GlobalContext.Limit.CalculateSL(SL2, SlLimitPips, orderType, price, _Symbol, spread);
	if(NormalizeDouble(SL2, digits) != NormalizeDouble(SL, digits))
   {
	...
	GlobalContext.Limit.CalculateTP(TP2, TpLimitPips, orderType, price, _Symbol, spread);
	if(NormalizeDouble(TP2, digits) != NormalizeDouble(TP, digits))
   {
	...
	GlobalContext.Limit.DeCalculateTP_SL(TP, SL, TpLimitPips2, SlLimitPips2, orderType, price, _Symbol, spread);
   ...
   if(NormalizeDouble(SlLimitPips2, digits) != NormalizeDouble(SlLimitPips, digits))
   {
	...
	TpLimitPips2 = 0.0;
	GlobalContext.Limit.DeCalculateTP(TP, TpLimitPips2, orderType, price, _Symbol, spread);
	if(NormalizeDouble(TpLimitPips2, digits) != NormalizeDouble(TpLimitPips, digits))
   {
      isOk = false;
	...
	SlLimitPips2 = 0.0;
	GlobalContext.Limit.DeCalculateSL(SL, SlLimitPips2, orderType, price, _Symbol, spread);
...
bool TestMoneyConversion(string &errors, bool verbose = false)
{
   ...
   double convertedPrice = money.CalculateCurrencyRateForSymbol(_Symbol, 0, 0, 0);
   if(convertedPrice == 0.0)
   {
      isOk = false;
      errors += "money.CalculateCurrencyPrice(_Symbol, 0, 0, 0)==0.0 ";
      if(verbose)
         printf("money.CalculateCurrencyPrice(_Symbol, 0, 0, 0)==0.0 ");
   }
